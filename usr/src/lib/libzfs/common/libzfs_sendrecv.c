...
#ifdef __APPLE__
	/*
	 * OS X - pass down offset since zfs_ioc_send()
	 * in the kernel can't get to it (no KPI).
	 */
	zc.zc_history_offset = lseek(outfd, 0, SEEK_CUR);
#endif /* __APPLE__ */
...
#ifdef __APPLE__
	/*
	 * OS X - set file's offset since zfs_ioc_send()
	 * in the kernel can't get to it (no KPI).
	 */
	(void) lseek(outfd, zc.zc_history_offset, SEEK_SET);
#endif /* __APPLE__ */
...
#ifdef __APPLE__
	/*
	 * OS X - pass down offset since zfs_ioc_recv()
	 * in the kernel can't get to it (no KPI).
	 */
	zc.zc_history_offset = lseek(infd, 0, SEEK_CUR);
#endif /* __APPLE__ */
...
#ifdef __APPLE__
	/*
	 * OS X - set file's offset since zfs_ioc_recv()
	 * in the kernel can't get to it (no KPI).
	 */
	(void) lseek(infd, zc.zc_history_offset, SEEK_SET);
#endif /* __APPLE__ */
