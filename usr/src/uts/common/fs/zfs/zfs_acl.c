...
zfs_acl_xform(znode_t *zp, zfs_acl_t *aclp)
{
	...
	aclp->z_ops = zfs_acl_fuid_ops;
#ifdef __APPLE__
	VERIFY(zfs_copy_ace_2_fuid(vnode_vtype(ZTOV(zp)), aclp, oldaclp,
...
#ifndef __APPLE__
	/* OS X - setting ACLs does not change the mode bits */
...
#ifdef __APPLE__
	enum vtype	vntype = vnode_vtype(ZTOV(zp));
#else
...
#ifdef __APPLE__
	/* OS X - inheritance is driven from VFS layer */
	if (aclp != NULL) {
...
// MacZFS TODO
#if 0
// TODO revisit this and merge signatures
	zfs_acl_t	*aclp;
	kauth_acl_t  k_acl;
...
// TODO this needs to be changed to vnode_isdir
	/*
